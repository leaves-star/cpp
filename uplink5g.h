#ifndef __UPLINK5G_H__
#define __UPLINK5G_H__
...
namespace hnc10 {
    ...
    class Uplink5G : public CommonTask {
    public:
        Uplink5G(std::string mqttUrl, std::string robotId, std::string robotKey);
        ~Uplink5G();
        ...
        void rcvCmd(DownlinkCmd5G &dcmd) {
            pthread_mutex_lock(&mutex_);
            dCmds.push_back(dcmd);
            pthread_mutex_unlock(&mutex_);
        }

        int getCmd(DownlinkCmd5G &dcmd) {
            if (dCmds.size() == 0) {
                return 0;
            }
            pthread_mutex_lock(&mutex_);
